...
class PlanoraController extends ChangeNotifier {
    ...
    if (rawPayments == null || rawPayments.isEmpty) {
      ...
      try {
        ...
        if (_payments.isEmpty) {
          _payments.addAll(_defaultPayments);
          await _savePayments();
        }
      } catch (_) {
        _payments
  ...
  String monthlyReportText() {
    ...
    if (categorySummary.isEmpty) {
      ...
      for (final category in categorySummary) {
        buffer.writeln(
          '- ${category.title}: ${formatMoney(category.used)} / ${formatMoney(category.limit)}',
        );
      }
    }
  ...
  static const List<PaymentItem> _defaultPayments = [
    PaymentItem(
      id: 'payment_internet',
      title: 'İnternet',
      category: 'Fatura',
