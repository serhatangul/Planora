...
class PlanoraController extends ChangeNotifier {
  ...
  Future<void> _loadCategories() async {
    ...
    try {
      ...
      if (_categories.isEmpty) {
        ...
        await _saveCategories();
      } else if (!_categories.contains('Alışveriş')) {
        final marketIndex = _categories.indexOf('Market');

        if (marketIndex >= 0) {
          _categories.insert(marketIndex + 1, 'Alışveriş');
        } else {
          final otherIndex = _categories.indexOf('Diğer');

          if (otherIndex >= 0) {
            _categories.insert(otherIndex, 'Alışveriş');
          } else {
            _categories.add('Alışveriş');
          }
        }
  ...
  String categoryLabel(String category) {
    ...
    final language = appLanguageCode
        .trim()
        .toLowerCase()
        .replaceAll('_', '-')
        .split('-')
    ...
    return labels[language] ?? labels['tr'] ?? category;
  }
  ...
  String defaultPaymentTitleLabel(String title) {
    ...
    return labels[language] ?? labels['tr'] ?? title;
  }
  ...
  String? _canonicalDefaultCategory(String category) {
    ...
    for (final entry in _defaultCategoryLabels.entries) {
      if (entry.value.values
          .any((label) => label.toLowerCase() == category.toLowerCase())) {
        return entry.key;
      }
    }
      ...
      if (category.used >= category.limit) {
        alerts.add(
          PlanoraAlert(
            ...
            title: _categoryAlertTitleText(
                lang, 'exceeded', categoryLabel(category.title)),
        ...
        alerts.add(
          PlanoraAlert(
            ...
            title: _categoryAlertTitleText(
                lang, 'near', categoryLabel(category.title)),
  ...
  List<String> get budgetHealthTips {
    ...
    if (notifyCategoryLimits && exceededCategories.isNotEmpty) {
      tips.add(_budgetTipText(lang, 'categoryExceeded',
          category: categoryLabel(exceededCategories.first.title)));
    }
    ...
    if (nearCategories.isNotEmpty) {
      tips.add(_budgetTipText(lang, 'categoryNear',
          category: categoryLabel(nearCategories.first.title)));
    }
    ...
    switch (code) {
      case 'en':
        return isExceeded
            ...
            : '$category limit is close';
      case 'ru':
        return isExceeded
            ? 'Лимит $category превышен'
            : 'Лимит $category почти достигнут';
      case 'tr':
      default:
        return isExceeded
            ? '$category limiti aşıldı'
            : '$category limiti yaklaşıyor';
    }
  }
